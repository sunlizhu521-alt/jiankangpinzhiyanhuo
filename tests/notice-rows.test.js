...
test('separate submissions only add records', () => {
  let rows = [];
  rows = mergeNoticeRowsById(rows, [{ id: 'notice-1', supplierShortName: '供应商A' }]);
  rows = mergeNoticeRowsById(rows, [{ id: 'notice-2', supplierShortName: '供应商B' }]);
  rows = mergeNoticeRowsById(rows, [{ id: 'notice-3', supplierShortName: '供应商C' }]);
...
test('rows without ids are appended without removing existing records', () => {
  ...
  const incoming = [{ supplierShortName: '供应商A' }];
